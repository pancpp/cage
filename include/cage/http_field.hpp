 ...
 * @date Oct 05, 2020
 */
#ifndef CAGE_HTTP_FIELD_HPP_
#define CAGE_HTTP_FIELD_HPP_
...
enum class HttpField : std::uint16_t {
  unknown,
  a_im,
  accept,
  accept_additions,
...
constexpr HttpField operator++(HttpField& x) {
  return x = static_cast<HttpField>(static_cast<std::uint16_t>(x) + 1);
}
...
constexpr HttpField begin(HttpField) {
  return HttpField::beg;
}

constexpr HttpField end(HttpField) {
  return HttpField::end;
}
